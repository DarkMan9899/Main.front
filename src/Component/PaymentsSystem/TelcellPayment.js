...
// Telcell վճարային համակարգի կարգավորումները
const TELCELL_CONFIG = {
    ...
    currency: '֏',
    valid_days: 10,
    shop_key: 'YOUR_TELCELL_SHOP_KEY' // Telcell-ի կողմից տրամադրված գաղտնի բանալին
};
...
// Telcell-ի security_code հաշվարկման ֆունկցիա
...
// Telcell վճարման ֆորմայի կոմպոնենտ
export function TelcellPaymentForm({ amount, orderId, product = "Polyglot Academy Purchase" }) {
    const formRef = useRef(null);
    const encodedProduct = base64encode(product);
    const encodedOrderId = base64encode(orderId || '');

    // Անվտանգության կոդի ստացում
    const securityCode = getTelcellSecurityCode(
        ...
        encodedProduct,
        encodedOrderId,
        TELCELL_CONFIG.valid_days
    );

    useEffect(() => {
        // Ավտոմատ ուղարկել ֆորման մոնտաժվելուց հետո
        if (formRef.current) {
...
// Telcell վճարման պրոցեսոր
export function TelcellPaymentProcessor({ paymentData, setPaymentStatus }) {
    useEffect(() => {
        // Ստուգել paymentData-ի առկայությունը
        if (!paymentData || !paymentData.orderId) {
            console.error('Missing required payment data for Telcell');
            setPaymentStatus({
                success: false,
                error: 'Order ID is required for Telcell payments'
            });
            return;
        }

        console.log('Telcell payment processor initialized with:', paymentData);
        // Ֆորմը ավտոմատ ուղարկվում է TelcellPaymentForm-ի useEffect-ի մեջ
